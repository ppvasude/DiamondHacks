...
## Report - Experiences and Learnings
 ...
 - iTrust :
    - We got an error with jetty as the port address was already in use. We couldn't understand why since we hadn't even set any port for it and it was running on the default port. We then checked and realised that this error was due to Jenkins server running on that same address. We then modified the command to start the jetty server and started it on a different port. We underestood how to confirm which port are assigned to which service and how to check active ports.
    - We got a build error due to the SMTP module for a long time. Each time it failed we confirmed that the credentials were correct and that we enabled the correct settings on out email account. But nothing seemed to be working; the build still failed. It was only when Professor asked us if we had some kind of antivirus installed on our system that we realised that our main machine had Avast. It helped us realise that even though we were working with virtual machines, we were still using the underlying hardware of our main machine.
